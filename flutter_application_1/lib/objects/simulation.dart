...
enum Field { U_FIELD, V_FIELD, S_FIELD }
...
class Fluid {
  ...
  Fluid(this.density, this.numX, this.numY, this.h) {
    ...
    for (int i = 0; i < numX; i++) {
      s[index(i, 0)] = 0.0;
      s[index(i, numY - 1)] = 0.0;
    }
    for (int j = 0; j < numY; j++) {
      s[index(0, j)] = 0.0;
      s[index(numX - 1, j)] = 0.0;
    }
    ...
    for (int iter = 0; iter < iterations; iter++) {
      for (int i = 1; i < numX - 1; i++) {
        for (int j = 1; j < numY - 1; j++) {
          ...
          final double neighboringFluid = sx0 + sx1 + sy0 + sy1;
          ...
          double correction = -div / neighboringFluid;
  ...
  double sampleField(double x, double y, Field field) {
    ...
    x0 = math.max(0, math.min(x0, numX - 1));

    y0 = math.max(0, math.min(y0, numY - 1));

    final int x1 = math.min(x0 + 1, numX - 1);

    final int y1 = math.min(y0 + 1, numY - 1);
  ...
  void advectVel(double dt) {
    ...
    for (int i = 1; i < numX - 1; i++) {
      for (int j = 1; j < numY - 1; j++) {
        if (s[i * n + j] != 0.0 && s[(i - 1) * n + j] != 0.0) {
          double x = i * h;
          double y = j * h + h2;

          final double velocityU = u[i * n + j];

          final double velocityV = avgV(i, j);

          x -= dt * velocityU;
          y -= dt * velocityV;

          final double sampledU = sampleField(x, y, Field.U_FIELD);
        ...
        if (s[i * n + j] != 0.0 && s[i * n + j - 1] != 0.0) {
          ...
          final double velocityU = avgU(i, j);

          final double velocityV = v[i * n + j];

          x -= dt * velocityU;
          y -= dt * velocityV;

          final double sampledV = sampleField(x, y, Field.V_FIELD);
  ...
  void advectSmoke(double dt) {
    ...
    for (int i = 1; i < numX - 1; i++) {
      for (int j = 1; j < numY - 1; j++) {
        ...
        final double velocityU = (u[i * n + j] + u[(i + 1) * n + j]) * 0.5;

        final double velocityV = (v[i * n + j] + v[i * n + j + 1]) * 0.5;
  ...
  void simulate(double dt, double gravityX, double gravityY, int iterations) {
  ...
  void addCircle(
    //adds air or solid depending on 'solid' parameter
    double x,
    double y,
    double radius,
    bool solid,
